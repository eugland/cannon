...
Verified with Unity 6000.0.82f1: 48 EditMode tests and 4 PlayMode tests pass, including `EveryLevel_IsWinnable_WithoutErrors`; Windows player build succeeds; launched player log contains no errors or exceptions. The level-editor server/API suite passes all 4 tests, including validation of the shipped 10-level catalog.

Level-editor usability pass: fixed canvas dimensions no longer change when the inspector opens; `Reset unsaved` restores the loaded/saved catalog; edge-aware 0.25/0.5/1-unit snapping aligns mixed-size castle pieces; four reusable castle assets ship with the catalog; tuned instances can become new reusable assets. Unity now exposes `Cannon > Level Editor` menu actions to launch the web editor, select source JSON, and validate runtime records. Verified with 7 model/API tests, a headless browser select/drag/reset regression, 48 EditMode tests, 4 PlayMode tests including every-level auto-win, Windows build, and clean player error/exception scan.
